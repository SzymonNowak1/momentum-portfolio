...
def detect_currency(ticker: str) -> str:
    """
    Bardzo proste reguły wykrywania waluty:
    - '.DE' -> EUR (niemiecka giełda, w tym ZPR1.DE)
    - '.PL' -> PLN (GPW)
    - domyślnie -> USD
    """
...
def build_target_allocation(
    ...
    - jeśli regime == 'BULL' -> równy podział między TOP5 (po 20%)
    - jeśli regime == 'BEAR' -> 100% w safe_asset (ZPR1.DE)
    ...
    if regime == "BULL" and top5 is not None and len(top5) > 0:
        ...
        # BEAR regime -> wszystko w bezpiecznym ETF ZPR1.DE
        ticker = safe_asset
        ccy = detect_currency(ticker)
        fx = fx_row[ccy]
